...
CREATE TABLE IF NOT EXISTS `match_gamer` (
  `GAMER_idGAMER` int(11) NOT NULL,
  `MATCH_idMATCH` int(11) NOT NULL,
  `Side` varchar(45) NOT NULL DEFAULT '',
  PRIMARY KEY (`GAMER_idGAMER`,`MATCH_idMATCH`),
  UNIQUE KEY `gamer_unique` (`GAMER_idGAMER`,`MATCH_idMATCH`,`Side`),
...
CREATE TABLE IF NOT EXISTS `match_score` (
  ...
  UNIQUE KEY `score_unique` (`SCORE_idSCORE`,`MATCH_idMATCH`,`Side`),
...
ALTER TABLE `match_score`
  ...
  ADD CONSTRAINT `match_score_ibfk_2` FOREIGN KEY (`SCORE_idSCORE`) REFERENCES `score` (`idScore`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `match_score_ibfk_3` FOREIGN KEY (`MATCH_idMATCH`) REFERENCES `match` (`idMATCH`) ON DELETE CASCADE ON UPDATE CASCADE;
